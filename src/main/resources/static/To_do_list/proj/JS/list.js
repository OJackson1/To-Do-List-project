...
function readOne(id){
    fetch('http://localhost:8905/task/read/'+id)
    .then(
      function(response) {
...
document.querySelector("form.todoForm").addEventListener("submit", function (stop) {
  ...
  function updatetodo(id,name){
    fetch("http://localhost:8905/task/update/"+id, {
        method: 'put',
        headers: {
  ...
  function deltodo(id){
    fetch("http://localhost:8905/taskItem/delete/"+id, {
        method: 'delete',
        headers: {
          "Content-type": "application/json"
        },
      ...
      .catch(function (error) {
        ...
        deldiv.textContent ="Error deleting Item";
      });
}
